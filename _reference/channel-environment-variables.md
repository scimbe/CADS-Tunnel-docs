...
| `CT_CHANNEL_RELAY` | Edge relay endpoint. Used on direct-dial failure when a direct path is attempted — but under `CT_CHANNEL_RELAY_ONLY=1` (below), there's no direct dial at all, and this is the primary (only) data path from the start. |
