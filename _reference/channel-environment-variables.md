...
| `CT_CHANNEL_BRIDGE_PEER` | 64-hex Noise pubkey (2026-09-01, ct-agent v0.7.19+). When set, exposes an extra tool tranche (`bridge/status`, `bridge/config`, `bridge/manifest-list`, `bridge/manifest-install`) — but **only** to a caller whose channel-authenticated identity matches this exact key, refused for every other admitted member. Reserved for the CADS-Tunnel portal's own remote-control feature (design in progress) — not something to set by hand for ordinary channel use. |
