...
| `ct-agent status` | **Yes** — `GET /status` from `CT_AGENT_METRICS_LISTEN` if it's set, printed as JSON; otherwise the last 20 events from the on-disk ring, with a note that no listener is configured. See [Environment variables]({{ '/reference/environment-variables/' | relative_url }})'s Observability section for the full `/status`/`/healthz`/`/events` surface. |
